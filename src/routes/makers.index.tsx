...
import { Hammer, Music2, Users, Award, ArrowRight } from "lucide-react";
    ...
    <PageShell>
      ...
      <section className="mt-12">
        ...
        <div className="mt-5 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {MAKERS.map((m, i) => (
            <motion.div key={m.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} whileHover={{ y: -4 }} className="rounded-2xl overflow-hidden border border-border/50 gradient-card shadow-card flex flex-col">
              ...
              <div className="p-5 flex flex-col flex-1">
                ...
                <p className="mt-3 text-sm text-foreground/80 line-clamp-3">{m.bio}</p>
                <div className="mt-3 inline-block self-start text-[10px] uppercase tracking-widest bg-gold/15 text-gold px-2 py-1 rounded-full">{m.expertise}</div>
